"""Visual keyboard for Pygame engine. Aims to be easy to use as highly
customizable as well.

``VKeyboard`` only require a pygame surface to be displayed on and a text
consumer function, as in the following example :
...
class VKeyboardRenderer(object):
    ...
    def draw_character_key(self, surface, key, special=False):
        ...
        pygame.draw.rect(surface, background_color[key.state],
...
class VKeyRow(object):
    ...
    def __init__(self, x_pos=0, y_pos=0):
        ...
        :param x_pos: Posição somada em x relativa ao centro calculado.
        :param y_pos: Posição somada em y relativa ao centro calculado.
        """
        self.keys = []
        self.y = -1
        self.height = 0
    ...
    def set_size(self, position, size, padding):
        ...
        """
        self.height = size
        self.position = position[0], position[1] + self.y_pos
        x = position[0] + self.x_pos
        for key in self.keys:
            key.set_size(size)
            # print((x, position[1]))
            key.position = (x, position[1] + self.y_pos)
...
class VKeyboardLayout(object):
    ...
    """ QWERTY layout. """
    QWERTY = ['1234567890', 'qwertyuiop', 'asdfghjklç', 'zxcvbnm']
    ...
    def __init__(self, model, key_size=None, padding=5, allow_uppercase=True, allow_special_chars=True, allow_space=True, x_pos=0, y_pos=0):
    ...
    def configure_specials_key(self, keyboard):
        ...
        """
        special_row = VKeyRow(self.x_pos, self.y_pos)
...
class VKeyboard(object):
    ...
    def __init__(self, surface, text_consumer, layout,
    ...
    def draw(self):
        ...
        if self.state > 0:
            self.renderer.draw_background(self.surface, (0, 0), (0, 0))
